...
export function createOllamaModelNode(container, startX = 500, startY = 300) {
  ...
  node.className =
    "group w-[100px] h-[100px] rounded-full bg-[#3b3a3a] border-2 border-gray-400 flex flex-col items-center justify-center relative shadow transition-all";
  ...
  deleteBtn.className =
    "absolute -top-8 left-1/2 -translate-x-1/2 w-6 h-6 flex items-center justify-center rounded-full bg-red-500 text-white border-2 border-white shadow z-50 font-bold text-lg opacity-0 group-hover:opacity-100 transition";
  deleteBtn.onclick = (e) => {
    e.stopPropagation();
  ...
  img.alt = "Ollama Model";
  img.className = "w-[60px] h-[60px] mb-2 rounded-full";
  node.appendChild(img);

  // Label
  const label = document.createElement("div");
  label.className =
    "absolute top-full left-0 w-full text-center mt-1 font-bold text-white text-base leading-tight pointer-events-none";
  label.style.minHeight = "2.4em";
  label.style.maxHeight = "2.4em";
  label.innerHTML = "Ollama<br>Model";
  node.appendChild(label);

  const topHandle = document.createElement("div");
  topHandle.className =
    "absolute left-1/2 -translate-x-1/2 -top-2 w-4 h-4 bg-gray-400 border-2 border-gray-400 flex items-center justify-center z-10 cursor-pointer transition";
  topHandle.style.transform += " rotate(45deg)";
  topHandle.dataset.nodeId = nodeId;
  topHandle.dataset.handleType = "target";
  topHandle.addEventListener("click", handleHandleClick);
