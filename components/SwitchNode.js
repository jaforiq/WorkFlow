...
export function createSwitchNode(container, startX = 500, startY = 300) {
  ...
  node.className =
    "group w-[110px] h-[100px] rounded-[16px] bg-[#3b3a3a] border-2 border-gray-400 flex flex-col items-center justify-center relative shadow transition-all";
  ...
  leftHandle.className =
    "absolute left-[-6px] top-1/2 -translate-y-1/2 w-3 h-5 bg-gray-400 border-2 border-gray-400 rounded-sm flex items-center justify-center z-10 cursor-pointer transition";
  leftHandle.dataset.nodeId = nodeId;
  leftHandle.dataset.handleType = "target";
  leftHandle.addEventListener("click", handleHandleClick);
